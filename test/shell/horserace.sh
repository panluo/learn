...
HORSE_RACE_TMP_DIR=$HOME/.horserace-`date +%s`-`head -c10 /dev/urandom | md5sum |
head -c30`
# 创建临时目录, 并移动到该目录下.
...
tput cup 0 0
# 画6条白线.
for n in `seq 5`; do  # 用这个80字符的字符串将终端变为彩色的.
    echo $BLANK80
done
# 将前景色设为黑色.
echo -ne '\E[30m'
...
while [ $COL -lt $WINNING_POS ]; do
    ...
    echo -ne '\E[30;42m'
    # 将光标移动到马的新位置.
    tput cup `expr $MOVE_HORSE + 5` `cat horse_${MOVE_HORSE}_position | head
    -1`
    # 画马.
    $DRAW_HORSE
    usleep $USLEEP_ARG
