...
draw_box(){
    ...
    # 开始画盒子
    count=1
    for (( r=$1; count<=$BOX_HEIGHT; r++)); do # 使用plot_char函数画垂直线.
        plot_char $r $2 $VERT
        let count=count+1
    done
    ...
    count=1
    for (( c=$2; count<=$BOX_WIDTH; c++)); do   # 使用plot_char函数画水平线.
        plot_char $1 $c $HORZ
        let count=count+1
    done
    count=1
