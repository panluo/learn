...
CONFPATH=$S3_HOME/job/logprocess/bilin_3.0.2_bidswitch.properties
...
function cmt_logprocess_step(){
	...
	--steps Type=CUSTOM_JAR,Name=logprocess_$3,ActionOnFailure=CONTINUE,Jar=$S3_JOB_HOME/logprocess/logProcess-bidswitch-3.0.3.jar,Args=com.bilin.main.Processor,$1,$2,$3,$4,$5) || return 1
...
function cmt_logprocess(){
	...
	do
		...
		local output_file=$(aws s3 ls $S3PATH/result/log_analysis/$i/$day/$hour |wc -l)
...
function proc_result_logprocess(){
	store2db "$1" $2 || return 1
	sync_camp_table $2 || return 2
}
...
function store2db(){
	...
	for log in $1;do
		...
		local result_file=$(aws s3 ls $S3PATH/result/log_analysis/$log/$day/$hour | wc -l)
		if [ $result_file -eq 0 ];then
			echo "$log result is not exists"
			continue
		fi
		
		aws s3 sync --quiet $S3PATH/result/log_analysis/$log/$day/$hour $result_path/$log/$day/$hour || return 1
