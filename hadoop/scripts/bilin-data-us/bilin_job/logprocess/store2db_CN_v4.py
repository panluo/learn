...
class StoreData:
    ...
    def convert_to_datetime(self, hours):
        ...
        """
        return datetime.datetime.fromtimestamp(((int)(hours) + 8) * 3600)
                ...
                if file_prefix == "bid":
                    ...
                    for line in open(self.file_path + files):
                        ...
                        data_tuple = [data[0],table_field[0], table_field[1], 
                                      ...
                                      table_field[5], table_field[6],table_field[14],
                                      (int)(data[2]), (int)(data[3]), data[1], data[4], 
                                      self.convert_to_datetime(data[5]), (int)(record[1]), (float)(record[2]), 'CN']

                        if data[0] == 'domain':
                            data_tuple += [self.get_top_domain(data[4])]
                            sql = 'insert into %s (%s, %s, %s, %s, %s, %s, %s, %s, top_domain) values (%d, %d, "%s", "%s", "%s", %d, %f, "%s", "%s")' % tuple(data_tuple)
                            self.rs_db.execute(sql)
                        else:
                            sql = 'insert into %s (%s, %s, %s, %s, %s, %s, %s, %s) values (%d, %d, "%s", "%s", "%s", %d, %f, "%s")' % tuple(data_tuple)
                            self.rs_db.execute(sql)
